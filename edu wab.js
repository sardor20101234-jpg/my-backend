...
import { Menu, X, BookOpen, GraduationCap, Users, Phone, Mail, MapPin, ArrowRight, Sparkles, Star, CheckCircle, Shield, Lightbulb, Globe, PlayCircle, Github, Linkedin, Twitter } from "lucide-react";
...
/*
  ...
  Pages:
   ...
   • Contact (working form UI + contact details)
...
const Button = ({ as: As = Link, to = "#", onClick, children, variant = "primary", size = "md", className = "", icon: Icon }) => {
  ...
  return As === "button" ? (
    ...
    <As to={to} className={classes}>{content}</As>
  );
};
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-zinc-100 bg-white/80 backdrop-blur">
      <Container className="flex h-16 items-center justify-between">
        ...
        <button onClick={() => setOpen(!open)} className="rounded-xl p-2 hover:bg-zinc-100 md:hidden">
    ...
    <Container className="grid grid-cols-1 gap-10 py-12 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <div>
        ...
        <form className="mt-3 flex gap-2">
          ...
          <Button as="button" variant="dark" onClick={(e)=>e.preventDefault()}>Subscribe</Button>
        </form>
      </div>
    </Container>
        ...
        <Container className="grid grid-cols-1 items-center gap-10 py-20 md:grid-cols-2">
          <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} transition={{duration:0.6}}>
            ...
            <div className="mt-6 flex items-center gap-2 text-sm text-zinc-500">
              <Star className="h-4 w-4 fill-yellow-400 stroke-yellow-400"/> 4.9/5 average rating • 120k+ learners
            </div>
          </motion.div>
              ...
              <motion.div key={name} initial={{opacity:0, y:20}} whileInView={{opacity:1, y:0}} viewport={{once:true}} transition={{duration:0.5, delay:0.08*i}}>
                <Card>
                  <div className="flex items-center gap-2 text-yellow-500">
                    {Array.from({length:5}).map((_,j)=>(<Star key={j} className="h-4 w-4 fill-yellow-400 stroke-yellow-400"/>))}
                  </div>
